...
SELECT * 
    ...
    INNER JOIN country_languages AS la ON (la.countries_code = co.code)
...
SELECT co.name AS "country", la.language, la.is_official
	FROM countries AS co
    INNER JOIN country_languages AS la ON (la.countries_code = co.code)
    WHERE 
		la.language = "Arabic" 
		AND la.is_official="T";

-- Get the population, city and country names where Chinese is spoken by less than 10% of the people.
SELECT ci.population, ci.name AS "city", co.name AS "country", la.language, la.percentage
	FROM cities AS ci
	JOIN countries AS co ON (ci.countries_code = co.code)
	JOIN country_languages AS la ON (la.countries_code = co.code)
    WHERE 
		la.language = "Chinese"
        AND la.percentage <= 10
